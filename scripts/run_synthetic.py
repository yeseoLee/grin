...
from lib import config, fillers
...
def run_experiment(args):
    ...
    dataset = ChargedParticles(
        static_adj=args.static_adj,
        window=args.window,
    ...
    np.fill_diagonal(adj, 0.0)  # force adj with no self loop
    ...
    if has_graph_support(model_cls):
        model_params = {"adj": adj, "d_in": dataset.n_channels, "d_u": dataset.n_exogenous, "n_nodes": dataset.n_nodes}
    else:
        model_params = {
            "d_in": (dataset.n_channels * dataset.n_nodes),
            "d_u": (dataset.n_channels * dataset.n_exogenous),
        }
    model_kwargs = parser_utils.filter_args(args={**vars(args), **model_params}, target_cls=model_cls, return_dict=True)

    # loss and metrics
    loss_fn = MaskedMetric(
        metric_fn=getattr(F, args.loss_fn), compute_on_step=True, metric_kwargs={"reduction": "none"}
    )
    ...
    filler_kwargs = parser_utils.filter_args(
        args={**vars(args), **additional_filler_hparams}, target_cls=filler_cls, return_dict=True
    )
