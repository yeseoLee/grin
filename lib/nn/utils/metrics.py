...
class MaskedMRE(MaskedMetric):
    def __init__(
        ...
        self.add_state("tot", dist_reduce_fx="sum", default=torch.tensor(0.0, dtype=torch.float))

    def _compute_masked(self, y_hat, y, mask):
        _check_same_shape(y_hat, y)
        val = self.metric_fn(y_hat, y)
        mask = self._check_mask(mask, val)
        val = torch.where(mask, val, torch.tensor(0.0, device=y.device, dtype=torch.float))
        y_masked = torch.where(mask, y, torch.tensor(0.0, device=y.device, dtype=torch.float))
